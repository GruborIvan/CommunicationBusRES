...
namespace WebClient
{
	public class InputRequestParser
	{
		...
		public static Response Parse(string userRequest, string query, string fields)
		{
			...
			{
				return new Response(5000, "BAD_FORMAT", $"Requested operation: {operation} is not supported!");
			}

			if(!int.TryParse(id, out int intResult) && !string.Equals(operation, ALLOWED_REQUESTS[1]))
			{
				return new Response(5000, "BAD_FORMAT", "Request parameter 'id' must be number!");
			}
			...
			if (!string.IsNullOrEmpty(query))
			{
				...
				{
					return new Response(5000, "BAD_FORMAT", "Wrong query 'name' parameter!");
				}

				if (!string.Equals("type", typeParts[0]) || !int.TryParse(typeParts[1], out int typeResult) )
				{
					return new Response(5000, "BAD_FORMAT", "Wrong query 'type' parameter!");
				}
			}
			...
			if(!string.IsNullOrEmpty(fields) && string.Equals(operation, ALLOWED_REQUESTS[0]))
			{
				...
				if(fieldsParts.Length > 0)
				{
					foreach(string filed in fieldsParts)
					{
						...
						{
							return new Response(5000, "BAD_FORMAT", $"Filed: {filed} not allowed!");
						}
					}
				} 
				else
				{
					return new Response(5000, "BAD_FORMAT", "Something is wrong with the 'fileds' parameter!");
				}
			}
